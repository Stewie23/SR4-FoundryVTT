...
export class CompendiumBrowser extends BaseClass {
    ...
    static override PARTS = {
        tabs: { template: "systems/sr4/dist/templates/apps/compendium-browser/tabs.hbs" },
        filters: { template: "systems/sr4/dist/templates/apps/compendium-browser/filters.hbs" },
        results: { template: "systems/sr4/dist/templates/apps/compendium-browser/results.hbs" },
        settings: { template: "systems/sr4/dist/templates/apps/compendium-browser/settings.hbs" },
    };
    ...
    private async _renderResultSlice(indexStart: number, indexEnd: number) {
        ...
        for (let idx = indexStart; idx < indexEnd; idx++) {
            ...
            const html = await foundry.applications.handlebars.renderTemplate(
                "systems/sr4/dist/templates/apps/compendium-browser/entries.hbs",
                { entry },
            );
    ...
    private async _renderSettings() {
        ...
        const context = { tree };
        const html = await foundry.applications.handlebars.renderTemplate(
            "systems/sr4/dist/templates/apps/compendium-browser/settings.hbs",
            context,
        );
